 ...
 * - Built via a class factory so another extension's editor can be the base:
 *   pi keeps only ONE editor factory (setEditorComponent replaces), so
 *   editor-installing extensions must stack classes through
 *   chainEditorComponent (editor-chain.ts), not replace each other.
 */
...
export function makeSlashSuggestEditorClass(Base: EditorConstructor = CustomEditor): EditorConstructor {
	// @ts-expect-error — runtime-overrides TS-private base methods
	// (isSlashMenuAllowed / isAtStartOfMessage / isInSlashCommandContext)
	return class SlashSuggestEditor extends Base {
...
export const SlashSuggestEditor = makeSlashSuggestEditorClass();
